      ...
      <div className="relative z-10 flex flex-col items-center text-center px-6 h-full" style={{ paddingTop: '150px' }}>
        ...
        >
          ...
          <span className="text-white/80 font-body text-sm font-medium">
            Introducing the ultimate Creator OS.
          </span>
        </motion.div>

        {/* Heading */}
        <BlurText 
          text="The Content OS Your Brand Deserves" 
          className="text-6xl md:text-7xl lg:text-[6.5rem] font-heading italic text-white leading-[0.85] max-w-4xl tracking-tight justify-center"
          delay={0.1}
          stagger={0.1}
        />
        ...
        >
          Draft once. Render locally. Publish everywhere. Clavio is the operational control room for modern creators, wildly reimagined.
        </motion.p>
        ...
        >
          <Link href="/signup" className="liquid-glass-strong hover:scale-105 transition-transform duration-300 rounded-full px-6 py-3.5 flex items-center gap-2 text-white font-medium text-sm">
            Get Started
            <ArrowUpRight className="w-4 h-4 opacity-70" />
          </Link>
          <Link href="/login" className="rounded-full px-6 py-3.5 flex items-center gap-2 text-white/80 hover:text-white transition-colors font-medium text-sm">
        ...
        <div className="mt-auto pb-12 pt-16 flex flex-col items-center w-full">
          ...
          >
            {/* Replaced Iconify with plain text / Lucide or simple SVG for now since iconify might not be installed */}
            {[
              { name: 'Instagram' },
              { name: 'TikTok' },
              { name: 'YouTube' },
