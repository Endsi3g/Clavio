...
export default async function AnalyticsPage() {
  ...
  if (queryError || postsResult?.error) {
    return <ErrorState title="Failed to load analytics" description={postsResult?.error?.message || queryError?.message || 'Unknown error'} />
  }

  const posts: Post[] = postsResult?.data ?? []
  const allMetrics: PostMetrics[] = metricsResult?.data ?? []

  // Build latest metrics per post
    ...
    <div className="space-y-6">
      ...
      <div className="border-b border-slate-200">
        <nav className="-mb-px flex gap-6" aria-label="Tabs">
          <a href="#" className="border-blue-500 text-blue-600 whitespace-nowrap border-b-2 py-4 px-1 text-sm font-medium">All Time</a>
          <a href="#" className="border-transparent text-slate-500 hover:border-slate-300 hover:text-slate-700 whitespace-nowrap border-b-2 py-4 px-1 text-sm font-medium">Weekly Review</a>
          <a href="#" className="border-transparent text-slate-500 hover:border-slate-300 hover:text-slate-700 whitespace-nowrap border-b-2 py-4 px-1 text-sm font-medium">Monthly Review</a>
        </nav>
      </div>
          ...
          <Card>
            ...
            <CardContent>
              {Object.keys(platformStats).length === 0 ? (
                <EmptyState
                  title="No data"
                  description="Publish posts to see platform breakdown."
                  className="py-4"
                />
              ) : (
